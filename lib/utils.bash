#!/usr/bin/env bash

set -euo pipefail

GH_REPO="https://github.com/stellar/stellar-cli"
TOOL_NAME="stellar-cli"
TOOL_TEST="stellar --version"

fail() {
	echo -e "asdf-$TOOL_NAME: $*"
	exit 1
}

curl_opts=(-fsSL)

# NOTE: You might want to remove this if stellar-cli is not hosted on GitHub releases.
if [ -n "${GITHUB_API_TOKEN:-}" ]; then
	curl_opts=("${curl_opts[@]}" -H "Authorization: token $GITHUB_API_TOKEN")
fi

sort_versions() {
	sed 'h; s/[+-]/./g; s/.p\([[:digit:]]\)/.z\1/; s/$/.z/; G; s/\n/ /' |
		LC_ALL=C sort -t. -k 1,1 -k 2,2n -k 3,3n -k 4,4n -k 5,5n | awk '{print $2}'
}

list_github_tags() {
	git ls-remote --tags --refs "$GH_REPO" |
		grep -o 'refs/tags/.*' | cut -d/ -f3- |
		sed 's/^v//'
}

list_all_versions() {
  # This will list only the last two major versions, always favoring non-rc
  # releases.
	list_github_tags | \
  tr ' ' '\n' | \
  sort -V | \
  awk -F. '
  {
      major_minor = $1 "." $2  # Capture the major and minor parts as the key
      if (!($0 ~ /-rc/) || !(major_minor in last)) {
          last[major_minor] = $0  # Prefer non-rc version or overwrite if no non-rc found yet
      }
  }
  END { for (i in last) print last[i] }
  ' | \
  sort -V |
  tail -n2
}

download_release() {
	local version filename url
	version="$1"
	filename="$2"
  targz=$(basename "$filename")
	url="$GH_REPO/releases/download/v${version}/${targz}"

	echo "* Downloading $TOOL_NAME release $version..."
	curl "${curl_opts[@]}" -o "$filename" -C - "$url" || fail "Could not download $url"
}

install_version() {
	local install_type="$1"
	local version="$2"
	local install_path="$3"

	if [ "$install_type" != "version" ]; then
		fail "asdf-$TOOL_NAME supports release installs only"
	fi

	(
		mkdir -p "$install_path"
		cp -r "$ASDF_DOWNLOAD_PATH"/* "$install_path"

		# TODO: Assert stellar-cli executable exists.
		local tool_cmd
		tool_cmd="$(echo "$TOOL_TEST" | cut -d' ' -f1)"
		test -x "$install_path/bin/$tool_cmd" || fail "Expected $install_path/$tool_cmd to be executable."

		echo "$TOOL_NAME $version installation was successful!"
	) || (
		rm -rf "$install_path"
		fail "An error occurred while installing $TOOL_NAME $version."
	)
}
